...
#
# Written by Ruby Byrne
# Adapted from fhd_check_aws.sh by Nichole Barry
#
# NOTE
...
ls_output_size=( $(aws s3 ls ${FHDdir}/output_data/ | tr -s ' ' | cut -d' ' -f3) )
ls_output_filename=( $(aws s3 ls ${FHDdir}/output_data/ | tr -s ' ' | cut -d' ' -f4) )
...
do
    ...
    then
        if [ -z ${miss_flag} ]
        then
            echo fits files smaller than specified min_size:
            miss_flag=1
        fi
...
do
    ...
    then
        if [ -z ${miss_flag} ]
        then
            echo Some fits files are missing:
            miss_flag=1
        fi
        echo $line
    fi

...
while true; do
    ...
    if [ -z $(curl -Is http://169.254.169.254/latest/meta-data/spot/termination-time | head -1 | grep 404 | cut -d \  -f 2) ]; then
        ...
        aws s3 cp ~/grid_out/fhd_job_aws.sh.o${JOB_ID} ${s3_path}/fhd_${version}/grid_out/fhd_job_aws.sh.o${JOB_ID}_${myip}.txt

        # Copy gridengine stderr to S3
        aws s3 cp ~/grid_out/fhd_job_aws.sh.e${JOB_ID} ${s3_path}/fhd_${version}/grid_out/fhd_job_aws.sh.e${JOB_ID}_${myip}.txt

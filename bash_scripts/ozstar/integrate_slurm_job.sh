...
if [ "$legacy" -ne "1" ]; then
    ...
    idl -IDL_DEVICE ps -IDL_CPU_TPOOL_NTHREADS $ncores -e integrate_healpix_cubes -args "$evenoddpol_file_paths" "$save_file_evenoddpol" &
    int_pids+=( $! )
    #***
...
#Integrate cubes
    ...
    idl -IDL_DEVICE ps -IDL_CPU_TPOOL_NTHREADS $ncores -e integrate_healpix_cubes -args "$even_file_paths" "$save_file_even" &
    int_pids+=( $! )
    idl -IDL_DEVICE ps -IDL_CPU_TPOOL_NTHREADS $ncores -e integrate_healpix_cubes -args "$odd_file_paths" "$save_file_odd" &
    int_pids+=( $! )
    wait ${int_pids[@]} # Wait for integration to finish before making PS
fi
